...
const getSortedSlotsByCompactness = (day, candidateSlots, branchId, timetable, timeSlots) => {
  ...
  const occupiedIndices = Object.keys(sectionSchedule)
    ...
    .filter((idx) => idx !== -1)
    .sort((a, b) => a - b); // Sort occupied indices

  const getSlotScore = (slot) => {
    const idx = timeSlots.indexOf(slot);
    if (idx === -1) return 10000;

    // If no slots are occupied yet, prioritize earlier slots
    if (occupiedIndices.length === 0) {
      return idx; // Lower index = earlier slot = lower score = higher priority
    }

    // Find the last occupied slot
    const lastOccupiedIdx = Math.max(...occupiedIndices);

    // PRIORITY 1: Slot immediately after the last occupied slot (to fill sequentially)
    if (idx === lastOccupiedIdx + 1) {
      return -1000; // Highest priority
    }

    // PRIORITY 2: Slots that fill gaps between occupied slots
    const hasGapBefore = occupiedIndices.some(oidx => oidx < idx);
    const hasGapAfter = occupiedIndices.some(oidx => oidx > idx);
    if (hasGapBefore && hasGapAfter) {
      // This slot is between two occupied slots, prioritize it to fill gaps
      return idx - 500;
    }

    // PRIORITY 3: Adjacent to any occupied slot
    const isAdjacent = occupiedIndices.some((oidx) => Math.abs(oidx - idx) === 1);
    if (isAdjacent) {
      return idx - 300;
    }

    // PRIORITY 4: Earlier slots (default)
    return idx;
  };
...
const TimetableGenerator = () => {
  ...
  // User Management - Now synced with Firestore
  const [users, setUsers] = useState([]);

  // Core Data - Now synced with Firestore in real-time
  ...
  // Subscribe to Firestore collections and auto-save changes
  ...
  useEffect(() => {
    ...
    // Users
    const unsubUsers = subscribeToArray(COLLECTIONS.USERS, (data) => {
      setUsers(data.length > 0 ? data : [
        ...
        { username: "teacher1", password: "teacher123", name: "Dr. Kumar", role: "Teacher", teacherId: "T001" },
        { username: "student1", password: "student123", name: "John Doe", role: "Student", assignedClass: "CS-A-SEM3" },
      ]);
    }, []);
    unsubscribers.push(unsubUsers);

    // Branches
    const unsubBranches = subscribeToArray(COLLECTIONS.BRANCHES, setBranches, []);
    unsubscribers.push(unsubBranches);

    // Subjects
    const unsubSubjects = subscribeToArray(COLLECTIONS.SUBJECTS, setSubjects, []);
    unsubscribers.push(unsubSubjects);

    // Teachers
    const unsubTeachers = subscribeToArray(COLLECTIONS.TEACHERS, setTeachers, []);
    unsubscribers.push(unsubTeachers);

    // Teacher-Subject Mappings
    const unsubMappings = subscribeToArray(COLLECTIONS.MAPPINGS, setTeacherSubjectMapping, []);
    unsubscribers.push(unsubMappings);

    // Classrooms
    const unsubClassrooms = subscribeToArray(COLLECTIONS.CLASSROOMS, setClassrooms, []);
    unsubscribers.push(unsubClassrooms);

    // Labs
    const unsubLabs = subscribeToArray(COLLECTIONS.LABS, setLabs, []);
    unsubscribers.push(unsubLabs);

    // Working Days
    const unsubWorkingDays = subscribeToArray(COLLECTIONS.WORKING_DAYS, setWorkingDays, []);
    unsubscribers.push(unsubWorkingDays);

    // Time Slots
    const unsubTimeSlots = subscribeToArray(COLLECTIONS.TIME_SLOTS, setTimeSlots, []);
    unsubscribers.push(unsubTimeSlots);

    // Breaks
    const unsubBreaks = subscribeToArray(COLLECTIONS.BREAKS, (data) => {
      setBreaks(data.length > 0 ? data : [
        { day: "All", startTime: "11:00", endTime: "11:30", type: "Short Break" },
        { day: "All", startTime: "13:30", endTime: "14:30", type: "Lunch Break" },
      ]);
    }, []);
    unsubscribers.push(unsubBreaks);

    // Half Days
    const unsubHalfDays = subscribeToArray(COLLECTIONS.HALF_DAYS, setHalfDays, []);
    unsubscribers.push(unsubHalfDays);

    // Counseling Periods
    const unsubCounseling = subscribeToArray(COLLECTIONS.COUNSELING, setCounselingPeriods, []);
    unsubscribers.push(unsubCounseling);
  ...
  // Auto-save to Firestore when data changes (with debouncing to avoid too many writes)
  useEffect(() => {
    if (!migrationChecked || users.length === 0) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.USERS, users);
    }, 500);
    return () => clearTimeout(timer);
  }, [users, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.BRANCHES, branches);
    }, 500);
    return () => clearTimeout(timer);
  }, [branches, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.SUBJECTS, subjects);
    }, 500);
    return () => clearTimeout(timer);
  }, [subjects, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.TEACHERS, teachers);
    }, 500);
    return () => clearTimeout(timer);
  }, [teachers, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.MAPPINGS, teacherSubjectMapping);
    }, 500);
    return () => clearTimeout(timer);
  }, [teacherSubjectMapping, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.CLASSROOMS, classrooms);
    }, 500);
    return () => clearTimeout(timer);
  }, [classrooms, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.LABS, labs);
    }, 500);
    return () => clearTimeout(timer);
  }, [labs, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.WORKING_DAYS, workingDays);
    }, 500);
    return () => clearTimeout(timer);
  }, [workingDays, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.TIME_SLOTS, timeSlots);
    }, 500);
    return () => clearTimeout(timer);
  }, [timeSlots, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.BREAKS, breaks);
    }, 500);
    return () => clearTimeout(timer);
  }, [breaks, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.HALF_DAYS, halfDays);
    }, 500);
    return () => clearTimeout(timer);
  }, [halfDays, migrationChecked]);

  useEffect(() => {
    if (!migrationChecked) return;
    const timer = setTimeout(() => {
      saveArrayToFirestore(COLLECTIONS.COUNSELING, counselingPeriods);
    }, 500);
    return () => clearTimeout(timer);
  }, [counselingPeriods, migrationChecked]);
  ...
  /**
   * Selects the best teacher from multiple mapped teachers for a subject
   * Uses workload-based selection to ensure balanced distribution
   * @param {Array} mappings - Array of teacher-subject mappings
   * @param {Array} teachers - Array of all teachers
   * @param {Object} teacherWorkload - Current workload for each teacher
   * @param {Object} subject - The subject being scheduled
   * @returns {Object|null} - Selected teacher object or null if none available
   */
  const selectTeacherForSubject = (mappings, teachers, teacherWorkload, subject) => {
    if (!mappings || mappings.length === 0) return null;

    // Get all teachers from the mappings
    const availableTeachers = mappings
      .map(m => teachers.find(t => t.id === m.teacherId))
      .filter(t => t !== undefined);

    if (availableTeachers.length === 0) return null;

    // If only one teacher, return that teacher
    if (availableTeachers.length === 1) return availableTeachers[0];

    // Select teacher with lowest workload ratio (current load / max load)
    const selectedTeacher = availableTeachers.reduce((best, current) => {
      const currentLoad = teacherWorkload[current.id] || 0;
      const bestLoad = teacherWorkload[best.id] || 0;

      const currentRatio = currentLoad / (current.maxLoad || 20);
      const bestRatio = bestLoad / (best.maxLoad || 20);

      // Prefer teacher with lower workload ratio
      // If equal, add some randomness for variety
      if (Math.abs(currentRatio - bestRatio) < 0.01) {
        ...
        for (const day of shuffledDays) {
          ...
          const sortedSlots = [...allSlots].sort((a, b) => {
            const sumScore = (slot) => participatingBranches.reduce((acc, bid) => {
              ...
              const occupiedIndices = Object.keys(sched)
                .filter(k => k.startsWith(`${day}-`))
                .map(k => timeSlots.indexOf(k.split("-")[1]))
                .filter(idx => idx !== -1)
                .sort((x, y) => x - y);
              ...
              // If no slots occupied, prioritize earlier slots
              if (occupiedIndices.length === 0) {
                return acc + idx;
              }

              // Find the last occupied slot for this branch
              const lastOccupiedIdx = Math.max(...occupiedIndices);

              // Highest priority: slot immediately after last occupied
              if (idx === lastOccupiedIdx + 1) {
                return acc + (idx - 1000);
              }

              // Check if adjacent to any occupied slot
              const isAdj = occupiedIndices.some(ox => Math.abs(ox - idx) === 1);
        ...
        for (const day of shuffledDays) {
          ...
          const sortedBlocks = [...candidateBlocks].sort((a, b) => {
            const score = (blk) => {
              ...
              // If no slots occupied, prioritize earlier blocks
              if (occupiedIndices.length === 0) {
                return startIdx;
              }
              ...
              // Highest priority: block immediately after last occupied slot
              if (startIdx === lastOccupiedIdx + 1) {
                return startIdx - 1000;
              }

              // Check if adjacent to any occupied slot
              const isAdj = occupiedIndices.some(ox => Math.abs(ox - startIdx) === 1 || Math.abs(ox - (startIdx + 1)) === 1);
              return startIdx - (isAdj ? 500 : 0);
            };
            return score(a) - score(b);
          });
  ...
  const generateTimetable = () => {
    ...
    shuffledBranches.forEach(branch => {
      shuffleArray(subjects.filter(s => branch.subjects.includes(s.id) && s.labHours > 0 && s.preferEarly && !s.isBasketCourse)).forEach(subject => {
        ...
        const teacher = selectTeacherForSubject(mappings, teachers, teacherWorkload, subject);
